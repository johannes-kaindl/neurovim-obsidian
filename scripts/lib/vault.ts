...
export function buildVault(spec: VaultSpec): string[] {
  ...
  // Der Generator ist repo-spezifisch und OPTIONAL: 3d-codeblocks erzeugt damit seine
  // Modelle, vim-dojo braucht keine erzeugten Assets — seine Missionsnotizen legt das
  // Plugin zur Laufzeit selbst an. Fehlt die Datei, ist das kein Fehler.
  // (Abweichung zur Vorlage, 2026-08-16 — dort war der Aufruf unbedingt.)
  const generator = join(fixtureDir, "make-assets.mjs");
  if (existsSync(generator)) {
    const erzeugt = execFileSync("node", [generator, vaultDir], { encoding: "utf-8" });
    log.push("Assets erzeugt:" + erzeugt.replace(/\n$/, "").replace(/\n/g, "\n   "));
  } else {
    log.push("kein fixture/make-assets.mjs — keine erzeugten Assets (in Ordnung)");
  }
