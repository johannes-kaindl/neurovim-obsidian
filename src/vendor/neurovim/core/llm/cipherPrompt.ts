/**
 * Pure prompt building for the CIPHER uplink. Platform-neutral.
 ...
 */
import { CHEATSHEET, type CheatsheetCategory } from '../data/cheatsheet';
...
/** Assemble CIPHER's knowledge base. The quick-reference body is passed in:
 *  the core cannot import @neurovim/content (the dependency runs the other
 *  way — content imports core types). Consumers that already hold ENTRIES use
 *  `quickReference(ENTRIES)`; others read it via ContentPort.getLore(). */
export function buildKnowledge(quickRefBody: string): CipherKnowledge {
  return { quickRef: quickRefBody, cheatsheet: serializeCheatsheet(CHEATSHEET) };
}
