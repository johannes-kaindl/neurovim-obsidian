...
4b. **The endpoint SOURCE belongs to the LLM Endpoint Manager once it is installed (Welle 8, 2026-09-25)** — `EndpointResolver` (`src/llm/endpointResolver.ts`) takes an optional third argument `{ manager, choice }` and resolves through the kit's `resolveEndpointSource` (capability `chat`, caller `neurovim`): manager first, else the local list. The chosen model rides on `config.model`, so `CorePortAdapter.forEndpoint` is unchanged; the manager path is NOT cached (the manager caches itself), the local path is, exactly as before. `findEndpointManager(app)` runs on EVERY resolve. `settings.choice` counts only with the manager (a model name is bound to its endpoint). `plugin.llmConfigured()` = manager installed OR every local endpoint has a model. The settings tab shows `buildEndpointSourceSection` (manager) or the local `buildEndpointList`; `reconnect()` goes through `plugin.resolveEndpointFresh()`. `tools/sync-kit.sh` knows `relayer_pure` for `endpoint-source` (cross-import onto `sampling-profiles`, vendored only as a dependency). GUI-Smoke R5-1..3 put a fake manager into the plugin slot and restore it in `finally` and in the abort handler.
